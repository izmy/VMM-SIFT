...
{			
    ...
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " imgInput.png keys1.txt " << std::endl
										  << "- imgInput.png: input images (in PNG format). " << std::endl
	...
	// the file argv[2] (so that the users can match the keypoints with their own matching algorithm if they wish to)
